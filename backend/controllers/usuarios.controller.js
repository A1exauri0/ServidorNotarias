...
async function inicializarTablasUsuarios(pool) {
  ...
  try {
    ...
    // 2. Insertar usuario administrador inicial si está vacío
    const [filasUsuarios] = await conexion.query(
...
async function intercambiarTurnos() {
  ...
  try {
    ...
    for (const u of usuarios) {
      ...
      if (nuevoTurno) {
        await conexion.query("UPDATE \`usuarios\` SET turno = ? WHERE id = ?", [
          nuevoTurno,
          u.id,
        ]);
        actualizados++;
