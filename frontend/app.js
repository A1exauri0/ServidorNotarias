...
function actualizarKpisYGraficas(datos) {
    ...
    datos.notarias.forEach(item => {
        ...
        notariasCombinadas[notariaNombre].pdfs += parseInt(item.total_pdfs || 0, 10);
        notariasCombinadas[notariaNombre].imagenes += parseInt(item.total_imagenes || 0, 10);

        // Agrupación por Volumen por Notaría
        if (!volumenesCombinados[claveVolumen]) {
            volumenesCombinados[claveVolumen] = { pdfs: 0, imagenes: 0 };
        }
        volumenesCombinados[claveVolumen].pdfs += parseInt(item.total_pdfs || 0, 10);
        volumenesCombinados[claveVolumen].imagenes += parseInt(item.total_imagenes || 0, 10);
    });
...
function renderizarGraficaVolumenes(datosVolumenes) {
    ...
    if (listaVolumenes.length > 15) {
        listaVolumenes.sort((a, b) => datosVolumenes[b].pdfs - datosVolumenes[a].pdfs);
...
function poblarDropdownsFiltro() {
    ...
    const usuarios = [...new Set(listaRegistrosLocal.map(r => r.usuario).filter(Boolean))].sort();
    const notarias = [...new Set(listaRegistrosLocal.map(r => r.notaria).filter(Boolean))].sort();
    const volumenes = [...new Set(listaRegistrosLocal.map(r => r.volumen).filter(Boolean))].sort();
...
function filtrarYRenderizarTabla(termino) {
    ...
    const registrosFiltrados = listaRegistrosLocal.filter(reg => {
        ...
        return coincideTexto && coincideUsuario && coincideNotaria && coincideVolumen;
    });
    ...
    registrosFiltrados.forEach(reg => {
        ...
        const iconoExportado = reg.exportado === 1
