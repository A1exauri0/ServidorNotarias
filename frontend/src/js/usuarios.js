 ...
 * Controla toda la interacción del CRUD, buscador  con la API local.
 */
...
async function inicializarVistaUsuarios() {
  // 1. Cargar listado inicial de usuarios en la tabla
  await cargarTablaUsuarios();

  // 2. Vincular el buscador en tiempo real de usuarios
  ...
  // 3. Vincular botones del modal y formulario
  ...
  // 4. Vincular botón de intercambiar turnos masivamente
...
async function guardarUsuario() {
  ...
  const nombreCompleto = document
    .getElementById("txtNombreCompleto")
    .value.trim();
  const nombreUsuario = document
    .getElementById("txtNombreUsuario")
    .value.trim();
  ...
  if (!nombreCompleto || !nombreUsuario || pin.length !== 4 || isNaN(pin)) {
    alert(
      "Por favor llene todos los campos obligatorios. El PIN debe ser exactamente de 4 dígitos numéricos.",
    );
    return;
  }
...
async function confirmarEliminarUsuario(id, nombre) {
  const confirmar = confirm(
    `¿Está seguro de que desea eliminar permanentemente al capturista "${nombre}"?`,
  );
  if (!confirmar) return;
...
async function intercambiarTurnosUsuarios() {
  const confirmar = confirm(
    "Esta acción cambiará los turnos de todos los usuarios registrados (de Matutino a Vespertino y viceversa).\n¿Desea continuar?",
  );
  if (!confirmar) return;

  try {
    const respuesta = await fetch(
      "http://localhost:3000/api/usuarios/intercambiar-turnos",
      {
        method: "POST",
      },
    );
